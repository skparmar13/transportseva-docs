...
interface ManagedRoute { id: string; origin: string; destination: string; distance: number; transit: string; status: 'Active' | 'Archived'; }

@Component({ selector: 'app-route-management', standalone: true, imports: [FormsModule, ModalComponent, TranslatePipe], changeDetection: ChangeDetectionStrategy.OnPush, templateUrl: './route-management.component.html' })
export class RouteManagementComponent {
  ...
  protected create(): void { this.form.set({ origin: '', destination: '', distance: '', transit: '' }); this.editing.set({ id: '', origin: '', destination: '', distance: 0, transit: '', status: 'Active' }); }
  protected edit(route: ManagedRoute): void { this.form.set({ origin: route.origin, destination: route.destination, distance: String(route.distance), transit: route.transit }); this.editing.set(route); }
  protected save(): void {
    ...
    this.routes.update((routes) => current.id ? routes.map((route) => route.id === current.id ? { ...route, origin: form.origin.trim(), destination: form.destination.trim(), distance, transit: form.transit.trim() } : route) : [{ id: `RT-${100 + routes.length + 1}`, origin: form.origin.trim(), destination: form.destination.trim(), distance, transit: form.transit.trim() || 'Not set', status: 'Active' }, ...routes]);
